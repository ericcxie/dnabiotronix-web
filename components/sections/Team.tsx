...
const Team: React.FC = () => {
  return (
    <div className="container mx-auto px-8 xl:px-20 py-8" id="team">
      <FadeUpMotionDiv>
      ...
      <FadeUpMotionDiv>
        <div className="flex flex-wrap justify-center">
          {teamMembers.map((member) => (
            <div
              key={member.name}
              className="p-4 md:w-1/3 lg:w-1/4 mx-0 lg:mx-4 xl:mx-10"
            >
              <div className="flex flex-col items-center md:items-start">
                <img
                  src={member.image}
                  alt={member.name}
                  className="w-[200px] sm:w-[150px] lg:w-[250px] rounded-2xl"
                />
                ...
                <ul className="text-left list-disc pl-5 md:pl-0 mt-2">
                  {member.description.split("\n").map((line, i) => (
                    <li
                      key={i}
                      className="text-gray-600 font-light text-sm sm:text-xs lg:text-sm"
                    >
                      {line}
                    </li>
                  ))}
